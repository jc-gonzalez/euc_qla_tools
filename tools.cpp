...
std::string preciseTimeTag()
{
    ...
    const auto nano_secs = std::chrono::duration<double, std::nano>(now - now_s).count() * .000001;

    char buffer[80];
    char ns[11];
    strftime(buffer, 80, "%Y%m%dT%H%M%S", loc_time);
    sprintf(ns, ".%09ld", long(nano_secs));
